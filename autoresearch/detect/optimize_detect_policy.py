...
def append_result_row(fixture_path: Path, metrics: dict, policy: dict):
    header = "timestamp\tfixture\tfinal_score\tprecision_at_k\tpairwise_accuracy\tgate_accuracy\treport_recall\tpolicy_json\n"
...
def main():
    parser = argparse.ArgumentParser(description="Search for better detect policy settings on a labeled fixture")
    ...
    parser.add_argument("--refresh-auto", action="store_true", help="Export a fresh auto-labeled fixture from the database")
    ...
    if baseline_metrics["labeled_positive"] == 0 or baseline_metrics["labeled_negative"] == 0:
        raise SystemExit(
            "not_enough_labels: need at least one positive and one negative labeled candidate in the fixture"
        )
    ...
    print(
        "baseline="
        f"{baseline_metrics['final_score']:.2f} "
