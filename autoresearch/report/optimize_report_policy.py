...
def main():
    parser = argparse.ArgumentParser(description="Optimize report policy using no-LLM simulations over recent report metrics")
    parser.add_argument("--fixture", default=str(DEFAULT_FIXTURE), help="Path to report fixture JSON")
    parser.add_argument("--limit", type=int, default=2, help="How many recent reports to include in the no-LLM optimize loop")
    parser.add_argument("--refresh-auto", action="store_true", help="Refresh the fixture from Postgres before optimization")
    parser.add_argument("--apply", action="store_true", help="Write the best policy to report_policy_config.json")
    parser.add_argument("--min-improvement", type=float, default=DEFAULT_MIN_IMPROVEMENT, help="Minimum average-score improvement required before applying")
